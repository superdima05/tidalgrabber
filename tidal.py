...
def start():
	...
	if int(mode) == 1:
		...
		for track in playlist:
			...
			name = translit(u""+track.name, "ru", reversed=True).encode("UTF-8")
                	...
                	album_name = translit(u""+track.album.name, "ru", reversed=True).encode("UTF-8")
                        ...
                        os.system('ffmpeg -i "music/'+name+'.flac" -y -metadata title="'+name+'" -metadata artist="'+artist_name+'" -metadata album="'+album_name+'" -c:a copy "music/'+name+'2.flac"')
			os.system('rm "music/'+name+'.flac"')
                	os.system('mv "music/'+name+'2.flac" "music/'+name+'.flac"') 
		want_start()
	elif int(mode) == 2:
		...
		url = session.get_media_url(track_id=track.id)
		name = translit(u""+track.name, "ru", reversed=True).encode("UTF-8")
		artist_name = translit(u""+track.album.name, "ru", reversed=True).encode("UTF-8")
		album_name = translit(u""+track.artist.name, "ru", reversed=True).encode("UTF-8")
		print(name+' - '+url)
                os.system('wget "'+url+'" -O "music/'+name+'.flac"'.encode('utf-8'))
                os.system('ffmpeg -i "music/'+name+'.flac" -y -metadata title="'+name+'" -metadata artist="'+artist_name+'" -metadata album="'+album_name+'" -c:a copy "music/'+name+'2.flac"')
		os.system('rm "music/'+name+'.flac"')
